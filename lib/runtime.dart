// Canonical runtime implementation (duplicated from transpiler/lib/runtime.dart to
// avoid fragile relative exports that broke after repo restructure). Any future
// changes should be mirrored in both places or refactored into a shared location.
...
class PohRuntime {
	...
	// ================= Collections Runtime =================
	// We model immutable-by-default list/map by returning List/Map but marking mutability with wrappers.
	// To keep things simple without custom types crossing function boundaries, we use Map metadata.
	// Lists: if mutable -> return a _PohList wrapper; else return an immutable List (we avoid enforcing deep immutability, but mutations only occur via helpers which check mutability).
	...
	static dynamic indexAt(dynamic container, dynamic index) {
			if (container is _PohList) {
				final i = _toIndex(index) - 1; // 1-based indexing
	...
	// Minimal basename helper (since we removed dependency on package:path for tests).
...
// Simple wrappers to mark mutable collections
class _PohList {
