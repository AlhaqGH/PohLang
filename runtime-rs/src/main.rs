...
#[command(name = "pohlangc", version, about = "PohLang compiler/runtime")] 
struct Args {
...
fn main() -> anyhow::Result<()> {
    ...
    if args.run {
        let mut vm = vm::Vm::with_base_dir(args.input.parent().unwrap_or_else(|| std::path::Path::new(".")).to_path_buf());
        vm.execute(&program)?;
        return Ok(());
    }
