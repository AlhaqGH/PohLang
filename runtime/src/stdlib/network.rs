...
pub fn json_stringify(value: &JsonValue) -> Result<String> {
    serde_json::to_string(value).map_err(|e| anyhow!("Failed to stringify JSON: {}", e))
}
...
pub fn json_stringify_pretty(value: &JsonValue) -> Result<String> {
    serde_json::to_string_pretty(value).map_err(|e| anyhow!("Failed to stringify JSON: {}", e))
}
...
pub fn json_length(json: &JsonValue) -> Result<usize> {
    match json {
        ...
        _ => Err(anyhow!(
            "Cannot get length of JSON value that is not array or object"
        )),
    }
}
