...
fn test_write_and_read_file() {
    ...
    let mut cmd = run_in_dir(&[
        &format!("Set r to write \"Hello PohLang!\" into file at \"{}\"", test_file_str),
        "Write \"File written\"",
    ], &temp_dir);
    cmd.assert().success().stdout(predicate::str::contains("File written"));
    ...
    cmd.assert().success().stdout(predicate::str::contains("Hello PohLang!"));
}
...
fn test_append_to_file() {
    ...
    let mut cmd = run_in_dir(&[
        &format!("Set r1 to write \"Line 1\" into file at \"{}\"", test_file_str),
        &format!("Set r2 to append \"Line 2\" into file at \"{}\"", test_file_str),
        &format!("Set r3 to append \"Line 3\" into file at \"{}\"", test_file_str),
        &format!("Set content to read file at \"{}\"", test_file_str),
        "Write content",
    ], &temp_dir);
    
    cmd.assert().success().stdout(predicate::str::contains("Line 1Line 2Line 3"));
}
...
fn test_file_exists() {
    ...
    cmd.assert().success().stdout(predicate::str::contains("File exists!"));
    ...
    cmd.assert().success().stdout(predicate::str::contains("File not found"));
}
...
fn test_delete_file() {
    ...
    cmd.assert().success().stdout(predicate::str::contains("File deleted"));
...
fn test_create_directory() {
    ...
    cmd.assert().success().stdout(predicate::str::contains("Directory created"));
...
fn test_copy_file() {
    ...
    let mut cmd = run_in_dir(&[
        &format!("Set r to copy file from \"{}\" to \"{}\"", source_str, dest_str),
        "Write \"File copied\"",
    ], &temp_dir);
    cmd.assert().success().stdout(predicate::str::contains("File copied"));
...
fn test_move_file() {
    ...
    let mut cmd = run_in_dir(&[
        &format!("Set r to move file from \"{}\" to \"{}\"", source_str, dest_str),
        "Write \"File moved\"",
    ], &temp_dir);
    cmd.assert().success().stdout(predicate::str::contains("File moved"));
